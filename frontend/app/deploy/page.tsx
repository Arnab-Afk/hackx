...
export default function DeployPage() {
  ...
  async function handleDeploy() {
    ...
    // Persist session ID to history in localStorage for the dashboard
    try {
  ...
  return (
    ...
    >
        ...
        {sessionId && (
          <Link
            ...
            className="text-xs px-3 py-1 rounded-sm transition-opacity hover:opacity-80"
              style={{ background: "#181818", color: "#9ca3af", border: "1px solid #1f2937" }}
          >
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        {/* Input area */}
        <div className="rounded-sm p-5 mb-6" style={{ background: "#181818", border: "1px solid #1f2937" }}>
        ...
        <button
          ...
          disabled={!canDeploy}
          className="w-full py-3 rounded-sm font-semibold text-xs uppercase tracking-widest transition-all disabled:opacity-40 disabled:cursor-not-allowed"
          style={{
            background: phase === "running" ? "rgba(255,255,255,0.08)" : "#e2f0d9",
            color: phase === "running" ? "#6b7280" : "#111111",
            border: "none",
          }}
        >
          {phase === "running" ? (
        ...
        {events.length > 0 && (
          <div className="mt-8">
            ...
            <div
              ref={streamRef}
              className="rounded-sm overflow-y-auto p-4 space-y-2"
        ...
        {/* Plan card */}
        {plan && (
          <div className="mt-6 rounded-sm p-5" style={{ background: "#181818", border: "1px solid #374151" }}>
            <div className="flex items-center gap-2 mb-3">
              <span className="text-lg">📋</span>
              <h3 className="font-semibold text-xs uppercase tracking-widest" style={{ color: "#d1d5db" }}>Deployment Plan</h3>
              <span
            ...
            <div className="flex gap-6 text-sm">
              <div>
                ...
                <div className="font-semibold" style={{ color: "#d1d5db", fontFamily: "var(--font-space-mono), monospace" }}>${plan.plan.estimated_cost_per_hour.toFixed(3)}/hr</div>
              </div>
        ...
        {phase === "done" && (
          <div className="mt-6 rounded-sm p-5" style={{ background: "#181818", border: "1px solid #1f2937" }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-semibold text-xs uppercase tracking-widest" style={{ color: "#22c55e" }}>Deployment Complete</h3>
            </div>
            <p className="text-sm" style={{ color: "#9ca3af" }}>{doneMsg}</p>
            {sessionId && (
              <Link
                href={`/sessions/${sessionId}`}
        ...
        {/* Error card */}
        {phase === "error" && (
          <div className="mt-6 rounded-sm p-4" style={{ background: "#181818", border: "1px solid #374151" }}>
            <div className="flex items-center gap-2 mb-1">
              <span className="w-1.5 h-1.5 rounded-sm" style={{ background: "#ef4444" }} />
              <h3 className="font-semibold text-xs uppercase tracking-widest" style={{ color: "#ef4444" }}>Deployment Failed</h3>
            </div>
            <p className="text-xs" style={{ color: "#9ca3af", fontFamily: "var(--font-space-mono), monospace" }}>{errMsg}</p>
          </div>
        )}
      </div>
    </div>
  ...
  if (evt.type === "action") {
    ...
    return (
      <div
        ...
        style={{
          background: hasError ? "#181818" : "#181818",
          border: `1px solid ${hasError ? "#374151" : "#1f2937"}`,
        }}
      >
        <div className="flex items-center gap-2 mb-1">
          <span>{icon}</span>
